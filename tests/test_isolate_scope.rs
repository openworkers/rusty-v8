...
fn isolate_scope_basic() {
  ...
  {
    let _scope = unsafe { v8::IsolateScope::new(&mut locker) };
...
fn isolate_scope_two_isolates_interleaved() {
  ...
  // Work on A
  {
    let _scope = unsafe { v8::IsolateScope::new(&mut locker_a) };
  ...
  {
    let _scope = unsafe { v8::IsolateScope::new(&mut locker_b) };
...
fn isolate_scope_three_isolates_round_robin() {
  ...
  for i in 0..2 {
    ...
    {
      let _scope = unsafe { v8::IsolateScope::new(&mut locker_c) };
...
fn isolate_scope_nested_same_isolate() {
  ...
  let _outer = unsafe { v8::IsolateScope::new(&mut locker) };
  {
    let _inner = unsafe { v8::IsolateScope::new(&mut locker) };
    let scope = pin!(v8::HandleScope::new(&mut *locker));
    let scope = &mut scope.init();
    let context = v8::Context::new(scope, Default::default());
    let scope = &mut v8::ContextScope::new(scope, context);
