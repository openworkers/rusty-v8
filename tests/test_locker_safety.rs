...
fn eval_js(locker: &mut v8::Locker, expr: &str) -> i64 {
  let _isolate_scope = unsafe { v8::IsolateScope::new(&mut **locker) };
...
// ---------------------------------------------------------------------------
// Test 8: HandleScope Without IsolateScope
// ---------------------------------------------------------------------------

/// After Locker::new(), try HandleScope::new() without creating an
/// IsolateScope first.
///
/// With the pure-mutex Locker, the isolate is NOT entered after Locker::new()
/// — `Isolate::GetCurrent()` does not return this isolate. However, V8's
/// HandleScope, Context::New, Script::Compile, and Script::Run all take an
/// explicit `Isolate*` parameter from the HandleScope/ContextScope chain.
/// They do not call `Isolate::GetCurrent()` in their hot paths, so simple
/// single-isolate operations happen to work.
///
/// **This is NOT a supported usage pattern.** Some V8 internal operations
/// (builtins, GC callbacks, etc.) may use `Isolate::Current()`. Always use
/// `IsolateScope` for correctness. This test documents the observed behavior.
#[test]
fn handle_scope_without_isolate_scope_documents_behavior() {
  let _setup_guard = setup();
  let mut isolate = v8::Isolate::new_unentered(Default::default());

  let mut locker = v8::Locker::new(&mut isolate);

  // Intentionally NO IsolateScope. The isolate is locked but not entered.
  // V8's HandleScope takes an explicit Isolate* — it does not check GetCurrent().
  let scope = pin!(v8::HandleScope::new(&mut *locker));
  let scope = &mut scope.init();
  let context = v8::Context::new(scope, Default::default());
  let scope = &mut v8::ContextScope::new(scope, context);

  // Simple JS works because Script::Compile/Run use the isolate from
  // the Context, not GetCurrent(). However, this is undocumented behavior
  // and callers MUST use IsolateScope for production code.
...
// ---------------------------------------------------------------------------
// Test 9: Reentrant Locker on Same Isolate
// ---------------------------------------------------------------------------

/// V8's C++ Locker supports recursive locking on the same thread.
/// Create Locker(A), then Locker(A) again (same isolate, same thread).
///
...
// ---------------------------------------------------------------------------
// Test 10: v8::Global Handles Across Locker Lifetimes
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Test 11: Non-LIFO Drop With Interleaved IsolateScope Work
// ---------------------------------------------------------------------------
...
fn non_lifo_drop_interleaved_isolate_scope_work() {
  ...
  let mut locker_a = v8::Locker::new(&mut isolate_a);
  let _isolate_scope = unsafe { v8::IsolateScope::new(&mut *locker_a) };
  let scope = pin!(v8::HandleScope::new(&mut *locker_a));
  let scope = &mut scope.init();
  let context = v8::Context::new(scope, Default::default());
  let scope = &mut v8::ContextScope::new(scope, context);

  let code = v8::String::new(scope, "999").unwrap();
  let script = v8::Script::compile(scope, code, None).unwrap();
  let result = script.run(scope).unwrap();
  assert_eq!(result.to_integer(scope).unwrap().value(), 999);
}
